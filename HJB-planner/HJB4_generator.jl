...
include("HJB4_functions.jl")

# 2) PARAMETERS --- --- ---

# vehicle parameters
marmot = Vehicle(1.5, 0.75, 1.0, 1.0, 0.475, 0.324, 0.5207, 0.2762, 0.0889)   
unit_car = Vehicle(1.0, 0.5, 1.0, 1.0, 0.5, 0.5, 0.75, 0.375, 0.125)   
veh = unit_car
...
if run_HJB == true
    ...
    max_steps = 5000
    anim_bool = true
    @time U_HJB = solve_HJB_PDE(du_tol, max_steps, env, veh, anim_bool)

    @save algs_path*"HJB-planner/bson/U_HJB4.bson" U_HJB
    @save algs_path*"HJB-planner/bson/env4.bson" env
    @save algs_path*"HJB-planner/bson/veh4.bson" veh
else
    @load algs_path*"HJB-planner/bson/U_HJB4.bson" U_HJB
end

N_grid = size(env.x_grid,1)*size(env.y_grid,1)*size(env.theta_grid,1)*size(env.v_grid,1)
